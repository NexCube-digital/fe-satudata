...
export default function RuanganFinancePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Filter Ruangan items (type === "ruangan" or category === "Rawat Inap" except base kategori)
      const ruanganItems = allItems.filter(
        (i) => i.type === "ruangan" || (i.category === "Rawat Inap" && i.type !== "kategori")
      );
      setServicePrices(ruanganItems);
    } catch (err) {
      console.error("Error fetching tarif ruangan", err);
      setFeedback({ type: "error", message: "Gagal memuat tarif Ruangan RS." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitForm = async (e) => {
    ...
    if (!formData.name.trim() || !formData.price) {
      setFeedback({ type: "error", message: "Nama kamar rawat inap dan nominal tarif wajib diisi." });
      return;
    }
    ...
    try {
      ...
      if (res?.success || res?.id) {
        setFeedback({
          type: "success",
          message: editingItem
            ? "Tarif kamar ruangan berhasil diperbarui!"
            : "Tipe/kelas kamar ruangan baru berhasil ditambahkan!",
        });
        setIsModalOpen(false);
        fetchData();
      } else {
        setFeedback({ type: "error", message: res?.message || "Gagal menyimpan biaya kamar ruangan." });
      }
    } catch (err) {
      console.error("Error saving ruangan price", err);
  ...
  const handleDelete = async (id, name) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus tipe kamar "${name}"?`)) return;

    try {
      const res = await deleteServicePrice(id);
      if (res?.success) {
        setFeedback({ type: "success", message: `Tipe kamar "${name}" berhasil dihapus.` });
        fetchData();
      } else {
        setFeedback({ type: "error", message: res?.message || "Gagal menghapus tipe kamar." });
      }
    } catch (err) {
      console.error(err);
  ...
  const vipCount = useMemo(() => servicePrices.filter((i) => i.name.toLowerCase().includes("vip") || i.name.toLowerCase().includes("utama")).length, [servicePrices]);
  const intensiveCount = useMemo(() => servicePrices.filter((i) => i.name.toLowerCase().includes("icu") || i.name.toLowerCase().includes("hcu")).length, [servicePrices]);
        ...
        <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between mb-8 w-full">
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3.5 py-1 text-xs font-bold text-cyan-800 mb-2">
                <Building2 className="h-3.5 w-3.5" /> Modul Master Keuangan RS • Kelola Ruangan
              </div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                Master Data Tarif Ruangan (Kategori: Rawat Inap)
              </h1>
              <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-2xl">
                Kelola katalog rincian tarif sewa per hari untuk seluruh tipe/kelas kamar ruangan yang terhubung langsung ke kategori <strong>Instalasi Rawat Inap</strong>.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0 sm:ml-auto">
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-700 to-teal-800 hover:from-cyan-800 hover:to-teal-900 text-white px-5 py-2.5 text-xs font-extrabold shadow-md hover:shadow-lg transition cursor-pointer whitespace-nowrap"
              >
                <Plus className="h-4 w-4" /> Tambah Kamar Rawat Inap
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
            <div className="rounded-3xl bg-white border border-slate-200/80 p-5 shadow-xs flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Kelas Kamar</p>
                <p className="text-2xl font-extrabold text-slate-900 mt-1">{servicePrices.length} Kamar</p>
              </div>
          ...
          <div className="rounded-3xl bg-white border border-slate-200/80 p-5 shadow-xs mb-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="relative col-span-2">
                ...
                <input
                  type="text"
                  placeholder="Cari nama tipe kamar (Kelas III, Kelas I, VIP, HCU, ICU) atau kode..."
                  value={searchTerm}
          ...
          <div className="rounded-3xl bg-white border border-slate-200/80 p-6 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
              <h3 className="text-base font-extrabold text-slate-900 flex items-center gap-2">
                ...
                Katalog Tarif Kamar Rawat Inap ({filteredPrices.length})
              </h3>
            ...
            {filteredPrices.length === 0 ? (
              <div className="py-12 text-center text-slate-400 italic text-xs">
                Tidak ada data tarif kamar ruangan yang cocok.
              </div>
            ) : (
                ...
                <table className="w-full text-center text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200 text-[10px] uppercase font-bold text-slate-400 tracking-wider bg-slate-50/50">
                      <th className="py-3.5 px-4 text-center">Terhubung Kategori</th>
                      <th className="py-3.5 px-4 text-center">Kode KPTL</th>
                      <th className="py-3.5 px-4 text-center">Nama Tipe / Kelas Kamar Rawat Inap</th>
                      <th className="py-3.5 px-4 text-center">Satuan</th>
                      <th className="py-3.5 px-4 text-center">Tarif Sewa Per Hari (Rp)</th>
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50/60 transition">
                          <td className="py-4 px-4 text-center whitespace-nowrap">
                            <span className="inline-flex items-center gap-1.5 rounded-xl bg-cyan-50 border border-cyan-200 px-2.5 py-1 text-xs font-extrabold text-cyan-800">
                              <FolderTree className="h-3.5 w-3.5 text-cyan-700" />
                              {item.category || "Rawat Inap"}
                            </span>
                          </td>
                          <td className="py-4 px-4 text-center font-mono font-bold text-cyan-900 whitespace-nowrap">
                            {item.kptl || item.code || "-"}
                          </td>
          ...
          {/* Modal Tambah/Edit Ruangan (Rawat Inap) */}
          {isModalOpen && (
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-6">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <h3 className="text-base font-extrabold text-slate-900 flex items-center gap-2">
                    ...
                    {editingItem ? "Edit Kamar Rawat Inap" : "Tambah Kamar Rawat Inap Baru"}
                  </h3>
                  <button
                ...
                <form onSubmit={handleSubmitForm} className="space-y-4 text-xs">
                  {/* Category Binding Badge */}
                  <div className="bg-cyan-50/70 p-3.5 rounded-2xl border border-cyan-200/80 space-y-1">
                    <label className="block text-xs font-extrabold text-cyan-900 mb-0.5">
                      Terhubung Ke Kategori *
                    </label>
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-cyan-300 text-xs font-extrabold text-cyan-900 shadow-2xs">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Kode Kamar / Ruangan</label>
                    <input
                      type="text"
                      placeholder="Contoh: RI-KL3, RI-VIP, RI-ICU"
                      value={formData.code}
                      onChange={(e) => setFormData({ ...formData, code: e.target.value })}
                      className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800 font-mono focus:border-cyan-600 focus:bg-white focus:outline-hidden"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Nama Tipe / Kelas Kamar Rawat Inap *</label>
                    <input
                      type="text"
                      required
                      placeholder="Contoh: Kamar Rawat Inap Kelas Utama / VIP, HCU, ICU"
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                      className="w-full rounded-2xl border border-cyan-200 bg-cyan-50/20 p-3 text-xs font-bold text-slate-900 focus:border-cyan-600 focus:bg-white focus:outline-hidden"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">Tarif Sewa Per Hari (Rp) *</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-2xl border border-slate-200 bg-slate-50 pl-10 pr-3 py-3 text-xs text-slate-900 font-mono font-bold focus:border-cyan-600 focus:bg-white focus:outline-hidden"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-2xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800 focus:border-cyan-600 focus:bg-white focus:outline-hidden"
                    >
                  ...
                  <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="rounded-2xl bg-gradient-to-r from-cyan-700 to-teal-800 hover:from-cyan-800 hover:to-teal-900 text-white px-5 py-2.5 text-xs font-bold shadow-md hover:shadow-lg transition cursor-pointer disabled:opacity-50 inline-flex items-center gap-2"
                    >
