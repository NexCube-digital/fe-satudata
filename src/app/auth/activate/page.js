    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200 p-8 shadow-xl text-center space-y-6">
        ...
        <Link href="/" className="inline-flex items-center gap-3.5 mx-auto">
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-r from-teal-700 to-cyan-800 p-2 text-white shadow-xs">
        ...
        {loading && (
          <div className="py-8 space-y-3">
            <Loader className="h-10 w-10 animate-spin text-teal-800 mx-auto" />
        ...
        {!loading && status === "success" && (
          <div className="space-y-4 py-4">
            <div className="h-16 w-16 rounded-full bg-emerald-50 text-[#16A34A] border border-emerald-200 flex items-center justify-center mx-auto">
            ...
            {user && (
              <div className="rounded-2xl bg-slate-50 p-4 border border-slate-200/80 text-left text-xs space-y-1">
                ...
                <p className="text-[10px] text-[#16A34A] font-bold uppercase pt-1">Status: Aktif ✓</p>
              </div>
            )}

            {needsPassword ? (
              <form onSubmit={handleSetPassword} className="space-y-4 text-left">
                <div className="rounded-xl bg-teal-50 border border-teal-100 p-3 text-[11px] text-teal-800 leading-normal font-medium">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-3 py-2 text-xs focus:border-teal-600 outline-none"
                  />
                </div>
                <div>
                ...
                {passMsg && (
                  <p className={`text-[10px] font-bold ${passMsg.includes("berhasil") ? "text-[#16A34A]" : "text-[#DC2626]"}`}>{passMsg}</p>
                )}
                <button
                  type="submit"
                  disabled={passLoading}
                  className="w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-700 to-cyan-800 hover:from-teal-800 hover:to-cyan-900 px-6 py-3 text-xs font-extrabold text-white shadow-md transition cursor-pointer disabled:opacity-50"
                >
              ...
              <Link
                href="/auth/login"
                className="w-full inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-700 to-cyan-800 hover:from-teal-800 hover:to-cyan-900 px-6 py-3 text-xs font-extrabold text-white shadow-md transition"
              >
                Masuk Sekarang
                <ArrowRight className="h-4 w-4" />
              </Link>
        ...
        {!loading && status === "error" && (
          <div className="space-y-4 py-4">
            <div className="h-16 w-16 rounded-full bg-red-50 text-[#DC2626] border border-red-200 flex items-center justify-center mx-auto">
              <XCircle className="h-10 w-10 text-[#DC2626]" />
            </div>

            <div className="space-y-1">
              <h3 className="text-xl font-extrabold text-slate-900">Aktivasi Gagal</h3>
              <p className="text-xs text-[#DC2626] font-medium max-w-xs mx-auto">{message}</p>
            </div>
            ...
            <form onSubmit={handleResend} className="rounded-2xl bg-slate-50 p-4 border border-slate-200 text-left space-y-2.5">
              ...
              <input
                ...
                className="w-full rounded-xl bg-white border border-slate-300 px-3 py-2 text-xs focus:border-teal-600 outline-none"
                required
              />
              <button
                type="submit"
              ...
              {resendMsg && (
                <p className="text-[10px] font-semibold text-[#16A34A] pt-1">{resendMsg}</p>
              )}
            </form>
