...
export function IgdTriaseForm({
	...
	const totalGCS = (Number(triaseData.gcs_e || 0) || 0) +
		(Number(triaseData.gcs_v || 0) || 0) +
		(Number(triaseData.gcs_m || 0) || 0);

	// Quick Preset Functions
	const applyGcsPreset = (e, v, m, statusText) => {
		const current = parseVitalSigns(entryDetail.igd_triase_data);
		const updated = {
			...current,
			gcs_e: String(e),
	...
	const applyVitalSignPreset = (presetType) => {
		const current = parseVitalSigns(entryDetail.vital_signs);
		let updated = { ...current };
						...
						<div className="p-4 bg-white rounded-xl border border-slate-200/90 shadow-2xs flex flex-col justify-between">
							...
							<div className="flex items-center gap-2 mt-2">
								<input
									...
									className="w-full rounded-xl border border-slate-300 px-3 py-2 text-xs text-slate-900 font-bold focus:outline-none focus:border-rose-600 focus:ring-2 focus:ring-rose-100"
								/>
				...
				<div className="p-5 space-y-4 text-xs">
					...
					<div>
						...
						<textarea
							...
							className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs text-slate-900 focus:border-rose-600 focus:ring-2 focus:ring-rose-100 focus:outline-none font-medium"
						/>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						<div>
							...
							<input
								...
								className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 focus:border-rose-600 focus:outline-none font-medium"
							/>
						</div>
						...
						<div>
							...
							{triaseData.has_allergy === "Ya" && (
								<input
									...
									className="w-full rounded-xl border border-rose-300 bg-rose-50/50 px-3 py-1.5 text-xs text-slate-900 focus:outline-none font-medium"
								/>
							)}
						</div>
					</div>
						...
						<div className="space-y-2.5">
							...
							<div className="grid grid-cols-4 gap-1.5">
								<div>
									...
									<input
										...
										placeholder="4"
										className="w-full text-center rounded-lg border border-slate-300 bg-white py-1 font-bold text-slate-900 focus:border-rose-600 focus:outline-none"
									/>
								</div>
								<div>
							...
							<div>
								...
								<input
									...
									className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1 text-xs font-semibold text-slate-900 focus:border-rose-600 focus:outline-none"
								/>
							</div>
						</div>
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-1">
									...
									<input
										...
										className="w-12 text-center rounded-lg border border-rose-300 bg-rose-50 text-rose-700 text-xs font-black py-0.5 focus:outline-none"
									/>
								</div>
							</div>
...
export function UgdDischargeSummary(props: any) {
	const { entryDetail = {}, onUpdateDetailField, type = "igd", field = { name: "ugd_discharge_status", label: "Ringkasan Kondisi Sebelum Meninggalkan UGD" }, onNavigateToRanap, onNavigateToRujuk, onNavigateToDeath } = props;
	...
	const updateExtraField = (key, val) => {
...
export default function FormIGD(props) {
	return (
