...
export default function FormRanap({
  ...
  const masterRoomOptions = useMemo(() => {
    ...
    const roomNames = Array.from(
      new Set(list.map((r: any) => r.name || r.value || r.label).filter(Boolean))
    );
    return roomNames.map((name: any) => ({
      value: name,
      label: name,
    }));
  }, [roomOptions]);

  const handleSelectMasterRoom = (selectedVal: string) => {
    handleFieldChange('selected_master_room', selectedVal);

    const list = roomOptions.length > 0 ? roomOptions : FALLBACK_ROOM_CATALOG;
    const found = list.find((r: any) => r.value === selectedVal || r.name === selectedVal || r.code === selectedVal);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3 border-t border-slate-200 text-xs font-semibold">
            <div className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-xl border border-slate-200">
              ...
              <input
                ...
                className="w-full bg-transparent font-bold text-slate-900 focus:outline-hidden"
              />
            </div>
            ...
            <div className="flex items-center justify-between bg-teal-50/60 px-3 py-2 rounded-xl border border-teal-200">
              ...
              <div className="flex items-center gap-1">
                {rmDigits.map((digit: string, idx: number) => (
                  <span
                    key={idx}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 font-semibold text-slate-900 focus:border-teal-700 focus:outline-hidden"
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:col-span-2">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-900 focus:border-teal-700 focus:outline-hidden"
                />
              </div>
              <div>
              ...
              <div className="flex items-center gap-4 text-xs font-semibold">
                ...
                {entryDetail.has_previous_admission && (
                  <input
                    ...
                    className="rounded-lg border border-slate-300 bg-slate-50 px-2.5 py-1 text-xs text-slate-900 focus:border-teal-700 focus:outline-hidden"
                  />
                )}
              </div>
            </div>
          ...
          <div className="space-y-4 text-xs">
            <div className="p-3.5 bg-teal-50/80 rounded-xl border border-teal-200/90 flex flex-wrap items-center justify-between gap-3 shadow-2xs">
              ...
              <div className="w-full sm:w-80">
                <ModernSelect
                  options={masterRoomOptions}
                  value={entryDetail.selected_master_room || entryDetail.room_type || ''}
                  onChange={(val: string) => handleSelectMasterRoom(val)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-2xs">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-900 focus:border-teal-700 focus:outline-hidden"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-900 focus:border-teal-700 focus:outline-hidden cursor-pointer"
                >
