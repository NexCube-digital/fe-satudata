...
export const LogsComponent: React.FC = () => {
  ...
  const [copiedTx, setCopiedTx] = useState<any>(null);
            ...
            <div className="flex flex-col md:flex-row gap-3 items-center justify-between p-4 border-b border-slate-100 bg-slate-50/60">
              <div className="relative w-full md:w-80">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-medium"
                />
              </div>
                      ...
                      <tr key={log.id} className="hover:bg-teal-50/40 transition-colors">
                        <td className="px-5 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-1.5 text-slate-700">
                            ...
                            <span>{log.created_at ? new Date(log.created_at).toLocaleString('id-ID') : '-'}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4 max-w-xs">
                          <p className="text-slate-800 truncate font-medium" title={log.information || log.TargetHospital?.name || ''}>
                            {log.information || log.TargetHospital?.name || '-'}
                          </p>
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          {log.tx_hash ? (
                            <div className="flex items-center gap-1.5">
                              ...
                              >
                                <span>{log.tx_hash.substring(0, 8)}...{log.tx_hash.substring(log.tx_hash.length - 6)}</span>
                              </TxHashLink>
                              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button onClick={() => setSelectedLog(null)} className="text-slate-400 hover:text-slate-600 text-lg font-bold cursor-pointer">
                ✕
              </button>
            </div>
