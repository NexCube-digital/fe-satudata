...
import { apiGet, apiPost, apiPut, getAvatarUrl } from '@/lib/api-client';

export interface UsersComponentProps {
  forcedRole?: string;
}

export const UsersComponent: React.FC<UsersComponentProps> = ({ forcedRole }) => {
  ...
  const [roleFilter, setRoleFilter] = useState<string>(forcedRole || 'all');
  const [statusFilter, setStatusFilter] = useState<string>('all');

  const [copiedId, setCopiedId] = useState<any>(null);
  const [actionLoadingId, setActionLoadingId] = useState<any>(null);
  const [showNikIds, setShowNikIds] = useState<Record<any, boolean>>({});
  const [selectedUserDetail, setSelectedUserDetail] = useState<any>(null);

  const toggleShowNik = (id: any) => {
    setShowNikIds((prev) => ({ ...prev, [id]: !prev[id] }));
  };

  const [isAddFaskesModalOpen, setIsAddFaskesModalOpen] = useState<boolean>(false);
  const [submittingFaskes, setSubmittingFaskes] = useState<boolean>(false);
  const [faskesForm, setFaskesForm] = useState<any>({
    name: '',
    email: '',
    password: '',
    medical_license: '',
  ...
  const [registeredFaskesCredentials, setRegisteredFaskesCredentials] = useState<any>(null);
  const [copiedCredentials, setCopiedCredentials] = useState<boolean>(false);

  const [isSendTokenModalOpen, setIsSendTokenModalOpen] = useState<boolean>(false);
  const [selectedHospitalForToken, setSelectedHospitalForToken] = useState<any>(null);
  const [tokenAmountToGrant, setTokenAmountToGrant] = useState<number>(10);
  const [isSubmittingToken, setIsSubmittingToken] = useState<boolean>(false);

  const handleOpenSendTokenModal = (u: any) => {
    setSelectedHospitalForToken(u);
      ...
      setUsersList([
        {
          ...
          wallet_address: '0x7E193C55...B89d',
        ...
        },
        {
          ...
          nik: 'KEMENKES-RSCM',
          status_account: 'active',
          wallet_address: '0x9F12DE33...A3BC',
        ...
        },
        {
          ...
          status_account: 'active',
          wallet_address: '0x5F81EE55...E2C4',
        ...
        },
        {
          ...
          nik: 'KEMENKES-ADMIN',
          status_account: 'active',
          wallet_address: '0x3F5BCB44...E21A',
  ...
  const handleAddFaskesSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
          ...
          {registeredFaskesCredentials && (
            <div className="bg-gradient-to-r from-emerald-500/15 via-emerald-600/10 to-teal-500/5 rounded-3xl border-2 border-emerald-500/30 p-6 shadow-xs mb-6 space-y-4">
              <div className="flex items-start gap-3">
                <div className="h-10 w-10 rounded-2xl bg-emerald-100 flex items-center justify-center text-emerald-800 border border-emerald-200 shrink-0">
              ...
              <div className="bg-white rounded-2xl border border-emerald-200 p-4 space-y-2 max-w-md text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-800 font-mono font-bold">{registeredFaskesCredentials.email}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Password Sementara:</span>
                  <span className="text-teal-900 font-mono font-bold bg-teal-50 px-2 py-0.5 rounded-lg border border-teal-100">
                    {registeredFaskesCredentials.password}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white text-xs font-bold transition shadow-xs cursor-pointer"
                >
              ...
              {!forcedRole && (
                <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl text-xs">
                  ...
                  <select
                    ...
                    className="bg-transparent font-semibold text-slate-800 cursor-pointer"
                  >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-1.5 mt-0.5">
                                  <span
                                    className={`inline-block px-2 py-0.5 text-[9px] font-bold rounded-md uppercase border ${
                                      u.role === 'admin'
                          ...
                          <td className="px-5 py-4">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              {u.nik && (
                                ...
                                >
                                  {showNikIds[u.id] ? <EyeOff className="h-3.5 w-3.5 text-teal-600" /> : <Eye className="h-3.5 w-3.5" />}
                                </button>
                              )}
                            </div>
                          </td>
          ...
          <div className="relative bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-2xl p-6 sm:p-8 flex flex-col">
            ...
            <form onSubmit={handleAddFaskesSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs focus:border-teal-800 outline-hidden bg-white text-slate-850"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-mono focus:border-teal-800 outline-hidden bg-teal-50 text-teal-900 font-bold"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-xs focus:border-teal-800 outline-hidden bg-white text-slate-850 cursor-pointer"
                  >
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                >
                  {submittingFaskes ? <Loader className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4" />}
                  Registrasi Faskes
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="relative bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-md p-6 flex flex-col">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">Jumlah Token</label>
                <input
                  type="number"
                  min="1"
                  value={tokenAmountToGrant}
                  onChange={(e) => setTokenAmountToGrant(Number(e.target.value))}
                  className="w-full px-3 py-2 border border-slate-200 rounded-xl text-xs focus:border-teal-600 text-slate-800"
                />
              </div>

              {errorMsg && <p className="text-xs text-[#DC2626] font-medium bg-rose-50 border border-rose-100 p-2 rounded-lg">{errorMsg}</p>}
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <button
                  ...
                  className="w-1/2 rounded-xl bg-gradient-to-r from-teal-700 to-cyan-800 text-white py-2.5 text-xs font-bold shadow-xs hover:from-teal-800 hover:to-cyan-900 transition flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="relative bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-lg p-6 sm:p-8 flex flex-col">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
              ...
              <button onClick={() => setSelectedUserDetail(null)} className="text-slate-400 hover:text-slate-700 font-bold cursor-pointer">
                ✕
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200 space-y-2.5">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-slate-900 font-mono font-bold">{maskNik(selectedUserDetail.nik)}</span>
