...
export const WorkflowSection: React.FC = () => {
  return (
    <section id="alur" className="glass-panel rounded-3xl border border-slate-200/80 bg-white px-6 py-10 shadow-xs sm:px-10 lg:px-14 lg:py-12">
      ...
      <div className="relative grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {workflowSteps.map((step, index) => (
          <div key={step.step} className="group relative flex flex-col items-start rounded-2xl border border-slate-200/80 bg-slate-50/50 p-6 shadow-2xs transition-all duration-300 hover:bg-white hover:border-teal-300 hover:shadow-md reveal-scale">
            <div className="flex items-center justify-between w-full">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-teal-200 bg-white text-xs font-extrabold text-teal-800 shadow-2xs group-hover:scale-105 transition duration-200">
                {step.step}
              </div>
              <span className="text-[10px] font-bold text-slate-400 font-mono">STEP {step.step}</span>
            </div>

            <h3 className="mt-5 text-sm font-extrabold text-slate-900 group-hover:text-teal-900 transition-colors">
              {step.title}
            </h3>
