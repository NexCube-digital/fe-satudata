        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {doctors.map((doctor) => (
            <div key={doctor.id} className="group flex flex-col">
              <div className="relative rounded-[28px] overflow-hidden aspect-[4/4.5] border border-slate-200/80 bg-slate-100 shadow-xs group-hover:shadow-md transition duration-300">
                <img
                  src={doctor.image}
                  alt={doctor.name}
              ...
              <div className="mt-3.5 space-y-1">
                <p className="text-xs font-extrabold text-teal-700 tracking-wide">{doctor.specialist}</p>
                <h3 className="text-base font-extrabold text-slate-900 line-clamp-1">{doctor.name}</h3>
              </div>
