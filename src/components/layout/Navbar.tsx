...
export default function Navbar({ user: initialUser, roleLabel, onLogout }: NavbarProps) {
  ...
  const handleDeleteNotif = async (e: React.MouseEvent, id: string | number) => {
    e.stopPropagation();
    e.preventDefault();
    try {
      ...
      <div className="mx-auto flex items-center justify-between px-5 py-2.5">
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="group flex items-center gap-3 transition-all duration-200">
            ...
            <div>
              <div className="text-base font-extrabold tracking-tight text-slate-900 group-hover:text-teal-800 transition-colors">Satu Data</div>
              <div className="text-[10px] font-semibold text-teal-700/80 tracking-wide uppercase">{roleLabel || "Dashboard"}</div>
            </div>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {tokenBalance !== null && (
            <div className="hidden sm:flex items-center gap-1.5 rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 px-3 py-1.5 text-xs font-bold text-[#D97706] shadow-xs">
              ...
              <span className="text-[9px] font-bold text-[#D97706]/80 uppercase tracking-wide">Token</span>
            </div>
          )}
          ...
          <div className="relative" ref={notifRef}>
            <button
              ...
              className={`relative flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl transition-all duration-200 ${
                isNotifOpen
                  ...
                  : "border border-slate-200 bg-slate-50/60 text-slate-500 hover:bg-teal-50 hover:text-teal-700 hover:border-teal-200 hover:shadow-sm"
              }`}
              aria-label="Notifikasi"
            >
              <Bell className="h-4.5 w-4.5" />
              {unreadCount > 0 && (
                <span className="absolute -right-1 -top-1 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-gradient-to-br from-teal-600 to-cyan-700 text-[9px] font-extrabold text-white ring-2 ring-white shadow-sm">
            ...
            {isNotifOpen && (
              <div className="absolute right-0 top-full mt-2.5 w-80 sm:w-96 rounded-3xl border border-slate-200/80 bg-white/95 backdrop-blur-xl p-0 shadow-2xl ring-1 ring-black/5 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                {/* Header */}
                <div className="bg-gradient-to-r from-teal-800 to-cyan-900 px-5 py-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-teal-200/80">Aktivitas terkini sistem</p>
                      </div>
                    </div>
                    {unreadCount > 0 && (
                      <button
                      ...
                      <Link
                        ...
                        className={`group flex items-start gap-3 rounded-2xl p-3 transition-all duration-150 border ${
                          n.read
                            ...
                            : "bg-teal-50/60 border-teal-200/60 hover:bg-teal-50 hover:border-teal-300/50"
                        }`}
                      >
                        <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border ${
                          n.read
                            ? "bg-slate-100 border-slate-200 text-slate-500"
                            : "bg-gradient-to-br from-teal-700 to-cyan-800 border-teal-600 text-white shadow-sm"
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-1 mb-0.5">
                            ...
                            {!n.read && <span className="h-1.5 w-1.5 rounded-full bg-teal-500 shrink-0" />}
                          </div>
                          <p className="text-[10px] text-slate-500 leading-relaxed pr-6">
                            <strong className="font-semibold text-teal-800">{n.actor}</strong> — {n.description}
                          </p>
                          <div className="flex items-center justify-between mt-0.5">
                            ...
                            <button
                              ...
                              className="text-slate-300 hover:text-[#DC2626] p-1 transition-colors rounded-md hover:bg-red-50"
                ...
                <div className="border-t border-slate-100 px-4 py-3 bg-slate-50/60 text-center">
                  <Link
                    ...
                    className="text-[11px] font-bold text-teal-800 hover:text-teal-700 inline-flex items-center gap-1.5 transition-colors"
                  >
          ...
          <div className="relative hidden sm:block" ref={dropdownRef}>
            <button
              ...
              className={`flex cursor-pointer items-center gap-2.5 rounded-2xl border px-2.5 py-1.5 transition-all duration-200 ${
                isDropdownOpen
                  ...
                  : "border-slate-200/80 bg-slate-50/60 hover:border-teal-200 hover:bg-teal-50/60"
              }`}
              aria-expanded={isDropdownOpen}
            >
              {/* Avatar */}
              <div className="relative h-7 w-7 overflow-hidden rounded-full bg-gradient-to-br from-teal-700 to-cyan-800 ring-2 ring-teal-500/20 shrink-0">
              ...
              <div className="hidden text-left sm:block">
                ...
                <div className="text-[9px] font-bold text-teal-700/80 uppercase tracking-wide">{displayRoleLabel}</div>
              </div>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 top-full mt-2.5 w-52 rounded-2xl border border-slate-200/80 bg-white/95 backdrop-blur-xl p-1.5 shadow-2xl ring-1 ring-black/5 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
                {/* Profile Header */}
                <div className="px-3 py-3 mb-1 bg-gradient-to-r from-slate-50 to-teal-50/30 rounded-xl border border-slate-100">
                ...
                <Link
                  ...
                  className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 transition-all hover:bg-teal-50 hover:text-teal-900"
                >
                ...
                <button
                  ...
                  className="flex w-full cursor-pointer items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-bold text-[#DC2626] transition-all hover:bg-red-50"
                >
