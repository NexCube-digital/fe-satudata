...
export const Setting: React.FC = () => {
  ...
  const [isNikFilledOnLoad, setIsNikFilledOnLoad] = useState<boolean>(false);
  const [phone, setPhone] = useState<string>('');
  const [address, setAddress] = useState<string>('');
  const [dateOfBirth, setDateOfBirth] = useState<string>('');
  const [sex, setSex] = useState<string>('L');
  ...
  const [statusAccount, setStatusAccount] = useState<string>('active');
  ...
  const [initialDateOfBirth, setInitialDateOfBirth] = useState<string>('');
  const [initialSex, setInitialSex] = useState<string>('L');
  const [initialNik, setInitialNik] = useState<string>('');
  ...
  const hasChanges =
    ...
    (user?.role === 'pasien' &&
      (dateOfBirth !== initialDateOfBirth || sex !== initialSex || (!isNikFilledOnLoad && nik !== initialNik))) ||
  ...
  const [passwordMsg, setPasswordMsg] = useState<{ type: string; text: string }>({ type: '', text: '' });
  ...
  const [walletMsg, setWalletMsg] = useState<{ type: string; text: string }>({ type: '', text: '' });
  const [walletBalance, setWalletBalance] = useState<string>('');

  // System Wallet (admin)
  const [systemWallet, setSystemWallet] = useState<any>(null);
  const [systemWalletBalance, setSystemWalletBalance] = useState<string>('');
  const [systemWalletLoading, setSystemWalletLoading] = useState<boolean>(false);
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setProfileMsg({ type: 'success', text: result.message || 'Email verifikasi berhasil dikirim ulang!' });
      } else {
    ...
    if (userData) {
      try {
        ...
        const passwordConfigured =
          localStorage.getItem(`passwordConfigured:${parsed.id}`) === 'true' ||
          parsed.passwordConfigured === true ||
  ...
  const handleCropSave = () => {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(img, x0 * canvasScale, y0 * canvasScale, renderWidth * canvasScale, renderHeight * canvasScale);
      ctx.restore();
  ...
  const fetchProfile = async (currentUser: any) => {
    ...
    const isHospital =
      currentUser?.role === 'rumah_sakit' || currentUser?.role === 'faskes' || currentUser?.role === 'staf_rs';
    ...
    try {
      ...
      if (res.ok && result.data) {
        ...
        const passwordConfigured =
          localStorage.getItem(`passwordConfigured:${currentUser?.id}`) === 'true' ||
          currentUser?.passwordConfigured === true ||
        ...
        if (u.profil) {
          ...
          setInitialPhone(u.profil.phone || '');
          setAddress(u.profil.address || '');
          setInitialAddress(u.profil.address || '');
          const pDob = u.profil.date_of_birth ? u.profil.date_of_birth.substring(0, 10) : '';
          setDateOfBirth(pDob);
          setInitialDateOfBirth(pDob);
          setSex(u.profil.sex || 'L');
          setInitialSex(u.profil.sex || 'L');
        } else if (u.hospitalProfile) {
          setPhone(u.hospitalProfile.phone || '');
          setInitialPhone(u.hospitalProfile.phone || '');
          setAddress(u.hospitalProfile.address || '');
          setInitialAddress(u.hospitalProfile.address || '');
          setLatitude(u.hospitalProfile.latitude || '');
          setInitialLatitude(u.hospitalProfile.latitude || '');
          setLongitude(u.hospitalProfile.longitude || '');
          setInitialLongitude(u.hospitalProfile.longitude || '');
        }
              ...
              <div className="flex flex-col items-center gap-4 w-full">
                ...
                <div className="flex gap-2 w-full">
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-teal-700 to-cyan-800 text-white font-bold text-xs transition cursor-pointer"
                  >
                    Ambil Foto
                  </button>
                  <button
            ...
            {sourceImage && (
              <div className="flex flex-col items-center gap-4 w-full">
                <div
                  ...
                  className="relative h-60 w-60 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shadow-inner cursor-grab active:cursor-grabbing select-none"
                >
                  <img
                    src={sourceImage}
                    alt="Source"
                ...
                <div className="flex items-center gap-3 w-full px-2">
                  ...
                  />
                  <span className="text-[10px] font-bold text-slate-600 font-mono">{zoom.toFixed(2)}x</span>
                </div>
