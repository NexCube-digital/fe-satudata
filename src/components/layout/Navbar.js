...
export default function Navbar({ user: initialUser, roleLabel, onLogout, fixed = false }) {
  ...
  useEffect(() => {
    if (!currentUser || currentUser.role !== "pasien") return;
    if (currentUser.patient_id || currentUser.patientId || currentUser.patient?.id) return;

    const syncPatientProfile = async () => {
      try {
        const result = await apiGet("/api/patient/profile");
        ...
        const updatedUser = { ...currentUser, patient_id: patientId };
        setCurrentUser(updatedUser);
        localStorage.setItem("user", JSON.stringify(updatedUser));
      } catch (error) {
        console.warn("Gagal memuat identitas pasien untuk Tanya AI:", error.message);
      }
    };
