...
import { apiGet, getAvatarUrl } from '@/lib/api-client';
...
export const LandingNavbar: React.FC<LandingNavbarProps> = ({ walletConnected, setWalletConnected }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      const endpoint =
        storedUser?.role === 'rumah_sakit' || storedUser?.role === 'faskes'
  ...
  const getNavLink = (linkHref: string) => {
            ...
            {links.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-4 right-4 h-[2px] rounded-full bg-gradient-to-r from-teal-700 to-cyan-600 transition-transform duration-300 ease-out origin-left ${
          ...
          <div className="hidden lg:flex items-center gap-3">
            {user ? (
              <div className="relative" ref={profileRef}>
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="text-xs font-extrabold text-slate-900">{user?.name || 'Pengguna'}</div>
              ...
              <div className="flex items-center gap-2 sm:gap-2.5">
                ...
                <Link
                  href="/login"
                  className="group relative inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-teal-700 via-teal-800 to-cyan-700 px-4.5 py-2 text-xs font-bold text-white shadow-md shadow-teal-900/25 hover:shadow-xl hover:shadow-teal-700/40 hover:-translate-y-0.5 transition-all duration-300 border border-teal-500/30 overflow-hidden"
                >
                  <span className="flex h-5 w-5 items-center justify-center rounded-lg bg-white/20 text-white shadow-xs group-hover:bg-white group-hover:text-teal-900 transition-all duration-300">
              ...
              {user ? (
                <div className="space-y-2">
                  ...
                  <Link
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-700 to-cyan-700 py-2.5 text-xs font-bold text-white shadow-xs"
                  >
