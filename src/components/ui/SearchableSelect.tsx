...
export interface SearchableSelectProps {
  ...
  options?: Array<SearchableOption | string | number>;
  placeholder?: string;
  searchable?: boolean;
  isLoading?: boolean;
  loadingText?: string;
...
export default function SearchableSelect({
  ...
  const normalizeStr = (v: any) => String(v ?? '');
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 border border-slate-200 bg-white text-slate-850 shadow-xs hover:border-teal-400 focus:border-teal-600 focus:outline-hidden transition-all disabled:opacity-60 cursor-pointer ${sizeClasses} ${className}`}
      >
        <span
            ...
            <div className="p-2 border-b border-slate-100 bg-slate-50/50">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white py-1.5 pl-8 pr-7 text-xs text-slate-800 outline-hidden focus:border-teal-500 font-medium"
                />
                {query && (
                  <button
                    type="button"
