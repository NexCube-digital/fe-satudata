...
export default function AudienceSwitcher() {
  ...
  return (
    <section id="panel" className="w-full my-2 mb-10 sm:mb-16 lg:mb-20 grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
      ...
      <div className="glass-panel rounded-3xl border border-slate-200/80 bg-white p-4.5 sm:p-8 shadow-sm reveal-left">
        <p className="text-[10px] sm:text-xs font-extrabold uppercase tracking-[0.35em] text-[#0D9488]">Pilih Konteks Pengguna</p>
        <h2 className="mt-1.5 sm:mt-2 text-xl sm:text-3xl font-extrabold text-slate-900 leading-tight tracking-tight">
        ...
        {/* Tab Buttons (Side-by-side on Mobile & Desktop) */}
        <div className="mt-4 sm:mt-6 flex flex-row gap-2 sm:gap-3">
          <button
            type="button"
            onClick={() => setActiveAudience("patient")}
            className={`flex-1 rounded-2xl border px-3 py-2.5 sm:px-4 sm:py-3.5 text-left transition-all duration-200 cursor-pointer ${
              activeAudience === "patient"
                ? "border-teal-300 bg-[#E6F4F1]/70 shadow-xs ring-1 ring-teal-300/50"
                : "border-slate-200 bg-slate-50/50 hover:bg-slate-100"
            }`}
          >
            <div className="flex items-center gap-2">
              <Stethoscope className={`h-4 w-4 sm:h-4.5 sm:w-4.5 ${activeAudience === "patient" ? "text-[#0D9488]" : "text-slate-500"}`} />
              <span className="text-xs font-extrabold text-[#334155]">Portal Pasien</span>
            </div>
            <p className="mt-1 text-[11px] font-medium text-[#64748B] hidden sm:block">Kendalikan persetujuan akses & lihat EHR.</p>
          </button>
          ...
          >
            <div className="flex items-center gap-2">
              <Building2 className={`h-4 w-4 sm:h-4.5 sm:w-4.5 ${activeAudience === "hospital" ? "text-[#0D9488]" : "text-slate-500"}`} />
              <span className="text-xs font-extrabold text-[#334155]">Portal RS / Faskes</span>
            </div>
            <p className="mt-1 text-[11px] font-medium text-[#64748B] hidden sm:block">Ajukan permohonan data & billing POS.</p>
          </button>
        </div>

        {/* Feature Checklists */}
        <div className="mt-4 sm:mt-6 space-y-1.5 sm:space-y-2.5 border-t border-slate-100 pt-3.5 sm:pt-5">
          <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#64748B]">Kemampuan Modul</p>
          {current.checklist.map((item) => (
            <div key={item} className="flex items-center gap-2 sm:gap-2.5 rounded-xl border border-teal-100 bg-[#E6F4F1]/40 px-3 py-1.5 sm:px-3.5 sm:py-2.5">
              <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#0D9488] shrink-0" />
              <span className="text-[11px] sm:text-xs font-bold text-[#334155]">{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-[#E2E8F0] bg-white p-4 sm:p-7 text-slate-900 shadow-md reveal-right">
        <div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${current.accent}`} />

        <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-3 sm:pb-4">
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-[0.3em] text-[#0D9488]">Simulator Aktif</span>
            <h3 className="text-base sm:text-lg font-extrabold text-[#334155]">{current.label}</h3>
          </div>
          <span className="rounded-full bg-emerald-50 border border-emerald-200 px-2.5 py-1 text-[9px] sm:text-[10px] font-extrabold text-emerald-700 flex items-center gap-1 shrink-0">
        ...
        {activeAudience === "patient" && (
          <div className="mt-4 sm:mt-5 space-y-4 sm:space-y-6">
            {/* Consent Controls */}
            <div>
              <p className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-[#64748B] mb-2.5 sm:mb-3 flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#0D9488]" />
                Izin Akses Rumah Sakit (Granular Consent)
              </p>
              <div className="space-y-2 sm:space-y-2.5">
                {/* RSCM */}
                <div className="flex items-center justify-between rounded-xl sm:rounded-2xl bg-[#E6F4F1]/60 border border-teal-200/80 px-3 py-2.5 sm:px-4 sm:py-3 gap-2">
                  <div className="min-w-0 flex-1">
                    <h5 className="text-[11px] sm:text-xs font-extrabold text-[#334155] truncate">RS Cipto Mangunkusumo</h5>
                    <p className="text-[9px] sm:text-[10px] text-[#64748B] truncate">Akses: Diagnosis & Alergi Obat</p>
                  </div>
                  <button
                    onClick={() => toggleConsent("rscm")}
                    className={`rounded-full px-2.5 py-1 text-[9px] sm:text-[10px] font-extrabold transition-all cursor-pointer shrink-0 ${
                      consents.rscm
                ...
                <div className="flex items-center justify-between rounded-xl sm:rounded-2xl bg-[#E6F4F1]/60 border border-teal-200/80 px-3 py-2.5 sm:px-4 sm:py-3 gap-2">
                  <div className="min-w-0 flex-1">
                    <h5 className="text-[11px] sm:text-xs font-extrabold text-[#334155] truncate">RS Harapan Kita (Jantung)</h5>
                    <p className="text-[9px] sm:text-[10px] text-[#64748B] truncate">Akses: Rekam Medis Jantung & Lab</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between rounded-xl sm:rounded-2xl bg-[#E6F4F1]/60 border border-teal-200/80 px-3 py-2.5 sm:px-4 sm:py-3 gap-2">
                  <div className="min-w-0 flex-1">
                    <h5 className="text-[11px] sm:text-xs font-extrabold text-[#334155] truncate">Laboratorium Kimia Farma</h5>
                    <p className="text-[9px] sm:text-[10px] text-[#64748B] truncate">Akses: Hasil Swab PCR & Vaksin</p>
                  </div>
                  <button
            ...
            <div>
              <p className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wider text-[#64748B] mb-2.5 sm:mb-3 flex items-center gap-1.5">
                <Clock className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#0D9488]" />
                Linimasa Medis Pasien Terpadu (Encrypted EHR)
              </p>
              <div className="relative border-l border-teal-200 ml-2 sm:ml-2.5 pl-3 sm:pl-4 space-y-3 sm:space-y-4">
                {/* Item 1 */}
                <div className="relative">
                  <span className={`absolute -left-[17px] sm:-left-[21px] top-1 h-2 sm:h-2.5 w-2 sm:w-2.5 rounded-full border border-white ${consents.rscm ? "bg-emerald-500" : "bg-[#0D9488]"}`} />
                  <div className="rounded-xl bg-white border border-slate-200/80 p-2.5 sm:p-3 shadow-2xs">
                    ...
                    {consents.rscm ? (
                      <div className="mt-1">
                        <h6 className="text-[11px] sm:text-xs font-extrabold text-[#334155]">Diagnosa: Infeksi Saluran Pernapasan (ISPA)</h6>
                        <p className="text-[9.5px] sm:text-[10px] text-[#64748B] mt-0.5 leading-snug">Dokter: dr. Amanda Sp.PD | Terapi: Amoxicillin 500mg, Paracetamol 500mg</p>
                      </div>
                    ) : (
                      <div className="mt-1 flex items-center gap-1.5 text-teal-600 text-[9.5px] sm:text-[10px] font-extrabold">
                ...
                {/* Item 2 */}
                <div className="relative">
                  <span className={`absolute -left-[17px] sm:-left-[21px] top-1 h-2 sm:h-2.5 w-2 sm:w-2.5 rounded-full border border-white ${consents.kimiaFarma ? "bg-emerald-500" : "bg-[#0D9488]"}`} />
                  <div className="rounded-xl bg-white border border-slate-200/80 p-2.5 sm:p-3 shadow-2xs">
                    <div className="flex items-center justify-between text-[9px] text-[#64748B]">
                    ...
                    {consents.kimiaFarma ? (
                      <div className="mt-1">
                        <h6 className="text-[11px] sm:text-xs font-extrabold text-[#334155]">Laporan Lab: Tes Kolesterol & Gula Darah</h6>
                        <p className="text-[9.5px] sm:text-[10px] text-[#64748B] mt-0.5 leading-snug">Hasil: Kolesterol 190 mg/dL (Normal), Gula Darah Puasa 95 mg/dL (Normal)</p>
                      </div>
                    ) : (
                      <div className="mt-1 flex items-center gap-1.5 text-teal-600 text-[9.5px] sm:text-[10px] font-extrabold">
                        <ShieldAlert className="h-3.5 w-3.5" />
