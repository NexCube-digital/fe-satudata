  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col pt-16 sm:pt-20">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 w-full space-y-10">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-[#0D9488]/40 bg-gradient-to-r from-[#0F766E] via-[#0D9488] to-teal-950 p-8 sm:p-12 text-white shadow-xl">
          <div className="pointer-events-none absolute -right-20 -top-20 h-80 w-80 rounded-full bg-teal-500/10 blur-[80px]" />

          <div className="relative z-10 max-w-3xl mx-auto text-center space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1 text-xs font-semibold text-teal-100">
            ...
            <div className="relative max-w-xl mx-auto pt-2">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-white/30 bg-white px-11 py-3.5 text-xs sm:text-sm text-slate-800 placeholder-slate-400 shadow-lg focus:outline-hidden focus:ring-2 focus:ring-teal-400"
              />
              {searchTerm && (
                <button
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 rounded-2xl px-4 py-2.5 text-xs font-extrabold transition cursor-pointer whitespace-nowrap ${
                  isActive
                    ? "bg-gradient-to-r from-[#0D9488] to-[#0F766E] text-white shadow-md shadow-teal-900/20"
                    : "bg-white border border-[#E2E8F0] text-[#64748B] hover:bg-[#E6F4F1] hover:text-[#0F766E]"
                }`}
              >
                <Icon className={`h-4 w-4 ${isActive ? "text-teal-100" : "text-slate-400"}`} />
                <span>{cat.label}</span>
              </button>
            );
          })}
        ...
        <div className="space-y-4">
          ...
          {filteredFaqs.length === 0 ? (
            <div className="rounded-3xl border border-[#E2E8F0] bg-white p-12 text-center space-y-3">
              <HelpCircle className="h-12 w-12 text-slate-300 mx-auto" />
              <h3 className="text-base font-bold text-[#334155]">Pertanyaan Tidak Ditemukan</h3>
              ...
              <button
                ...
                className="mt-2 inline-flex items-center gap-1.5 rounded-xl bg-[#E6F4F1] border border-teal-200 px-4 py-2 text-xs font-bold text-[#0D9488]"
              >
                Reset Pencarian
              </button>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl border transition-all duration-200 overflow-hidden ${
                      isOpen
                        ? "border-teal-300 bg-white shadow-md ring-1 ring-teal-300/50"
                        : "border-[#E2E8F0] bg-white/90 hover:border-slate-300 shadow-2xs"
                    }`}
                  >
                    <button
                      type="button"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="rounded-lg bg-[#E6F4F1] text-[#0D9488] border border-teal-100 px-2 py-0.5 text-[10px] font-bold">
                          {faq.categoryLabel}
                        </span>
                        <h3 className="text-xs sm:text-sm font-extrabold text-[#334155]">
                          {faq.question}
                        </h3>
                      </div>
                      <ChevronDown
                    ...
                    {isOpen && (
                      <div className="px-5 pb-5 pt-0 text-xs leading-relaxed text-[#64748B] border-t border-slate-100 mt-1 pt-3 animate-in fade-in duration-200 space-y-3">
                        <p>{faq.answer}</p>
        ...
        <div className="pt-6 border-t border-[#E2E8F0]">
          <div className="text-center max-w-xl mx-auto mb-8">
            <h3 className="text-xl font-extrabold text-[#334155]">Belum Menemukan Jawaban?</h3>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-2xs hover:shadow-md transition space-y-3">
              <div className="h-10 w-10 rounded-xl bg-[#E6F4F1] text-[#0D9488] flex items-center justify-center border border-teal-100">
                <Mail className="h-5 w-5" />
              </div>
              <h4 className="text-sm font-bold text-[#334155]">Bantuan Email Teknis</h4>
              <p className="text-xs text-[#64748B]">Kirimkan pertanyaan atau kendala integrasi API Faskes ke tim teknis kami.</p>
              <a href="mailto:support@satudata.id" className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0D9488] hover:underline">
