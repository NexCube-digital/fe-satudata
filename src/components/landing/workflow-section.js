  ...
  return (
    <section id="alur" className="glass-panel rounded-[2.5rem] bg-white px-6 py-12 shadow-[0_20px_80px_rgba(225,29,72,0.03)] sm:px-10 lg:px-14 lg:py-16">
      ...
      <div className="max-w-3xl mb-12">
        <p className="text-xs font-bold uppercase tracking-[0.35em] text-rose-600">Alur Keamanan Data</p>
      ...
      <div className="relative grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {workflowSteps.map((step, index) => (
          <div key={step.step} className="group relative flex flex-col items-start rounded-3xl border border-rose-100 bg-rose-50/15 p-6 transition-all duration-350 hover:bg-rose-50/40 hover:border-rose-200">
            {/* Step Bubble Indicator */}
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-rose-200 bg-white text-sm font-extrabold text-rose-600 shadow-xs shadow-rose-100 transition-transform duration-300 group-hover:scale-105">
              {step.step}
            </div>

            {/* Content */}
            <h3 className="mt-6 text-sm font-bold text-slate-900 group-hover:text-rose-600 transition-colors">
              {step.title}
            </h3>
            ...
            {index < workflowSteps.length - 1 && (
              <div className="absolute -right-3.5 top-9 z-10 hidden text-rose-300 lg:block animate-pulse">
