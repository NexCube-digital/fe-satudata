  ...
  const normalizedOptions = useMemo<OptionItem[]>(() => {
    return options.map((opt) => {
      if (typeof opt === 'object' && opt !== null) {
        ...
        return {
          value: typeof item.value === 'object' ? String((item.value as any)?.value || '') : item.value,
          label: item.label ?? String(item.value),
          badge: item.badge,
          disabled: item.disabled || false,
        };
      }
      return { value: opt as string | number, label: String(opt), disabled: false };
    });
  }, [options]);
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 border border-slate-200 bg-white text-slate-800 shadow-xs hover:border-teal-400 focus:border-teal-600 focus:outline-none transition-all disabled:opacity-60 cursor-pointer ${sizeClasses} ${className}`}
      >
        <span
          ...
          <div className="overflow-y-auto max-h-52 p-1.5 space-y-0.5 custom-scrollbar">
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((opt) => {
                const isSelected = normalizeStr(opt.value) === normalizeStr(value);
                return (
                  <button
